...
export default function ProductCard({ product }) {
  ...
  return (
    <div
      ...
      className="relative bg-medi-panel border border-accent/20 rounded-lg p-3 flex flex-col cursor-pointer hover:shadow-accentGlowLg hover:border-accent/50"
    >
      {discount > 0 && (
        <span className="absolute top-2 left-2 z-10 bg-urgent-gradient text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
          {discount}% OFF
        </span>
      )}
      <button
      ...
      >
        <Heart size={16} className={wishlisted ? 'fill-urgent-from text-urgent-from' : 'text-gray-300'} />
      </button>
      ...
      {stock > 0 && stock <= 5 && (
        <p className="text-xs text-urgent-from font-medium mb-2">Only {stock} left!</p>
      )}
      {stock === 0 && (
      ...
      <button
        ...
        className="mt-auto bg-accent hover:bg-accent-light hover:scale-[1.02] text-black text-sm font-semibold py-2.5 rounded-full disabled:opacity-60 disabled:hover:scale-100 transition-all"
      >
