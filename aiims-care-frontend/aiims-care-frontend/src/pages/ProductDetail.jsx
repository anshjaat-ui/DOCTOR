...
function SpinViewer({ images }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black/60 text-accent text-[10px] px-3 py-1 rounded-full">
        Drag to rotate 360°
      </span>
    </div>
  )
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} type="button" onClick={() => setRating(n)}>
          <Star size={22} className="text-accent" fill={n <= rating ? '#D4AF37' : 'none'} strokeWidth={1.5} />
        </button>
      ))}
    </div>
  )
...
export default function ProductDetail() {
  ...
  function loadProduct() {
    ...
    api
      ...
      .then(async (data) => {
        ...
        const reviewList = await api.get(`/reviews?product=${data._id}`)
  ...
  if (loading) {
    return <main className="bg-medi-bg min-h-[70vh] flex items-center justify-center text-gray-400">Loading product...</main>
  }

  if (error || !product) {
    return (
      <main className="bg-medi-bg min-h-[70vh] flex flex-col items-center justify-center gap-3 text-gray-400">
        <p>Product nahi mila.</p>
        <button onClick={() => navigate('/')} className="text-accent hover:underline text-sm">Home pe wapas jao</button>
      </main>
    )
  }
  ...
  return (
    <main className="bg-medi-bg min-h-[70vh] font-sans">
        ...
        <div>
          ...
          {images.length >= 3 && (
            <button
              ...
              className="mb-3 text-xs text-accent border border-accent/30 rounded-full px-3 py-1 hover:bg-accent/10 transition-colors"
            >
        ...
        <div>
          <div className="flex justify-between items-start">
            ...
            <button onClick={handleWishlist} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center shrink-0" aria-label="Wishlist">
              <Heart size={20} className={wishlisted ? 'fill-urgent-from text-urgent-from' : 'text-gray-300'} />
            </button>
          </div>
          ...
          <p className={`text-sm mb-4 font-medium ${stock > 5 ? 'text-green-400' : stock > 0 ? 'text-urgent-from' : 'text-gray-500'}`}>
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-accent hover:bg-accent-light text-black font-semibold px-8 py-3 rounded-full text-sm disabled:opacity-50 transition-colors"
          >
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 border-t border-accent/10">
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-4 mb-8">
            {reviews.map((r) => (
              <div key={r._id} className="bg-medi-panel border border-accent/10 rounded-lg p-4">
        ...
        <div className="bg-medi-panel border border-accent/10 rounded-lg p-4 max-w-md">
          <h3 className="text-sm font-medium text-white mb-3">Write a review</h3>
          {reviewError && (
            <div className="bg-urgent-from/10 border border-urgent-from/30 text-urgent-from text-xs px-3 py-2 rounded mb-3">
              {reviewError}
            </div>
          )}
          <form onSubmit={handleReviewSubmit} className="space-y-3">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-black/40 border border-accent/30 text-white rounded-md px-3 py-2 text-sm"
            />
            <button
              type="submit"
              disabled={submittingReview}
              className="bg-accent hover:bg-accent-light text-black text-sm font-medium px-5 py-2 rounded-full disabled:opacity-60"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {related.map((p) => (
              <Link
                key={p._id}
                to={`/product/${p._id}`}
                className="bg-medi-panel border border-accent/20 rounded-lg p-3 hover:border-accent/50 transition-colors"
              >
