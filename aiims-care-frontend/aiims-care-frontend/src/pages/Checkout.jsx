  ...
  return (
    <main className="bg-medi-bg min-h-[70vh] font-sans">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 px-4 py-6">
        <form onSubmit={handlePayment} className="lg:col-span-2 bg-medi-panel border border-accent/20 p-5 rounded-lg space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input name="fullName" required placeholder="Full name" value={address.fullName} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 col-span-2" />
            <input name="phone" required placeholder="Phone number" value={address.phone} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 col-span-2" />
            <input name="line1" required placeholder="Address line 1" value={address.line1} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 col-span-2" />
            <input name="line2" placeholder="Address line 2 (optional)" value={address.line2} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 col-span-2" />
            <input name="city" required placeholder="City" value={address.city} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5" />
            <input name="state" required placeholder="State" value={address.state} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5" />
            <input name="pincode" required placeholder="Pincode" value={address.pincode} onChange={handleChange} className="bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 col-span-2" />
          </div>
          ...
          <div className="border-t border-accent/10 pt-3">
            <label className="text-sm font-medium text-gray-300 flex items-center gap-1 mb-2">
              <Tag size={14} className="text-accent" /> Have a coupon?
            </label>
            {appliedCoupon ? (
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-black/40 border border-accent/30 text-white rounded-sm px-2 py-1.5 text-sm"
                />
                <button
                  type="button"
                  onClick={handleApplyCoupon}
                  disabled={applyingCoupon}
                  className="bg-white/5 hover:bg-white/10 border border-accent/30 text-accent px-4 rounded-sm text-sm disabled:opacity-60"
                >
                  {applyingCoupon ? 'Checking...' : 'Apply'}
                </button>
              </div>
            )}
            {couponError && <p className="text-xs text-urgent-from mt-1">{couponError}</p>}
          </div>
